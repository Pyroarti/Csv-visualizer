...
# It promts the user to select a csv file and adds the csv files to a list for handling and plotting multiple csv data.
...
# Takes the csv data and makes them into dataframes and adds the componentes to a list for the user to select what to plot.
...
def create_dash_app(components_name, data, checked):
    ...
    app.layout = html.Div([
        ...
        dcc.RangeSlider( # HTML for the hour slider so the user can select the hour of the day to see the plottet data.
...
class App(customtkinter.CTk):
    def __init__(self, *args, **kwargs):
        ...
        button_explore = customtkinter.CTkButton(master=self.frame_1, command=lambda: browse_files(self), text="Browse files")
